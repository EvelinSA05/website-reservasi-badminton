...
$user = ensure_login_and_role(['admin', 'owner']);
$flash = pull_flash();
$canCreateCourt = user_has_permission($user, 'court.create');
$canUpdateCourt = user_has_permission($user, 'court.update');
$canDeleteCourt = user_has_permission($user, 'court.delete');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'create') {
            ensure_permission($user, 'court.create', 'lapangan.php');
            $id = next_prefixed_id('lapangan', 'id_lapangan', 'LP');
            $nama = trim($_POST['nama_lapangan'] ?? '');
            $jenis = trim($_POST['jenis_lantai'] ?? '');
            $hargaRaw = trim((string) ($_POST['harga_per_jam'] ?? ''));
            if ($hargaRaw === '' || !preg_match('/^\d+$/', $hargaRaw)) {
                redirect_with_flash('lapangan.php', 'error', 'Harga per jam harus berupa angka bulat.');
            }
            $harga = (int) $hargaRaw;
        ...
        if ($action === 'update') {
            ensure_permission($user, 'court.update', 'lapangan.php');
        ...
        if ($action === 'delete') {
            ensure_permission($user, 'court.delete', 'lapangan.php');
            $id = trim($_POST['id_lapangan'] ?? '');
...
<head>
    ...
    <title>Master Lapangan | <?= h($company['short_name']) ?></title>
    ...
    <div class="panel-wrap">
        <div class="panel">
            <div class="panel-title">
                <div>
                    <h2 style="font-size: 28px;">Master Lapangan <?= h($company['short_name']) ?></h2>
                    <p class="panel-subtitle">Kelola data lapangan, status operasional, dan harga sewa per jam untuk <?= h($company['name']) ?>.</p>
                </div>
        ...
        <?php if ($canCreateCourt): ?>
            <div class="panel">
                <h3 style="margin-top: 0;">Tambah Lapangan Baru</h3>
                <p class="desc-muted" style="margin-top:-4px; margin-bottom:14px;">Lapangan yang ditambahkan di sini akan menjadi bagian dari katalog internal <?= h($company['short_name']) ?>.</p>
                <form method="POST" class="form-reset">
                    ...
                    <div class="grid">
                        ...
                        <input class="input" type="text" name="harga_per_jam" placeholder="Harga per jam" inputmode="numeric" pattern="[0-9]*" required>
                        ...
                        <?php foreach ($rows as $row): ?>
                            <tr class="lap-row"
                                ...
                                <td class="action-cell">
                                    ...
                                    <?php else: ?>
                                        <span class="desc-muted">Read only</span>
                                    <?php endif; ?>
                                </td>
                            </tr>
                            <tr class="row-form" id="form-<?= h($row['id_lapangan']) ?>"<?= $canUpdateCourt ? '' : ' style="display:none;"' ?>>
                                <td colspan="5">
                                    <?php if ($canUpdateCourt): ?>
                                        <form method="POST" class="form-reset">
                                            ...
                                            <div class="grid-2">
                                                ...
                                                <input class="input" type="text" name="harga_per_jam" value="<?= h($row['harga_per_jam']) ?>" inputmode="numeric" pattern="[0-9]*" required>
                                                <select class="select" name="status" required>
    ...
    <script>
        ...
        function applyFilter() {
            ...
            rows.forEach((row) => {
                ...
                const formId = editButton ? editButton.dataset.target : '';
                const formRow = formId ? document.getElementById(formId) : null;
                if (formRow && !visible) {
