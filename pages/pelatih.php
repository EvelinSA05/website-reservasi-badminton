...
$relatedCoaches = array_values(array_filter($coaches, static function (array $item) use ($slug): bool {
...
<head>
    ...
    <title><?= htmlspecialchars(($coach['name'] ?? 'Pelatih tidak ditemukan') . ' | ' . $company['short_name']) ?></title>
    ...
    <style>
        :root { --bg-dark:#110022; --bg-dark-2:#330044; --text:#e2e8f0; --muted:#94a3b8; --line:rgba(255,255,255,.12); --lime:#a3e635; }
        * { box-sizing:border-box; }
        ...
        .main { padding:24px; }
        .eyebrow { display:inline-flex; align-items:center; gap:8px; padding:7px 12px; border-radius:999px; background:rgba(163,230,53,.14); color:#bef264; font-size:12px; font-weight:700; }
        ...
        .focus { margin-top:20px; padding:18px; border-radius:22px; border:1px dashed rgba(255,255,255,.22); background:rgba(20,8,30,.46); }
        .focus strong { display:block; margin-bottom:10px; color:#bef264; text-transform:uppercase; letter-spacing:.14em; font-size:12px; }
        .chips { display:flex; gap:8px; flex-wrap:wrap; }
        .chips span { padding:8px 12px; border-radius:999px; background:rgba(255,255,255,.06); border:1px solid var(--line); font-size:13px; }
        ...
        .side-link span { display:block; margin-top:6px; color:#bef264; font-size:13px; }
...
<div class="container">
    <div class="top">
        <header class="nav">
            ...
            <div class="nav-actions">
                <a href="/index.php#pelatih" class="btn-outline">Kembali ke Pelatih</a>
                <?php if (!$user): ?>
                    <a href="/login.php" class="btn">Login</a>
                <?php elseif ($isPelanggan): ?>
                    <a href="/pages/reservasi.php?coach=<?= urlencode($coach['slug'] ?? '') ?>" class="btn">Booking Coach</a>
                <?php else: ?>
    ...
    <section class="shell">
        <?php if (!$coach): ?>
            <div class="empty">
                <h1>Pelatih tidak ditemukan</h1>
                <p>Data coach yang diminta belum tersedia.</p>
            </div>
        <?php else: ?>
            <div class="layout">
                <article class="main">
                    <span class="eyebrow">Cari Pelatih</span>
                    <h1><?= htmlspecialchars($coach['name']) ?></h1>
                    <p class="lede"><?= htmlspecialchars($coach['summary']) ?></p>
                    <div class="meta">
                        <span><?= htmlspecialchars($coach['specialty']) ?></span>
                        <span><?= htmlspecialchars($coach['level']) ?></span>
                        <span><?= htmlspecialchars($coach['price']) ?></span>
                        <span>&#11088; <?= htmlspecialchars($coach['rating']) ?></span>
                        <span><?= htmlspecialchars($coach['availability']) ?></span>
                    </div>

                    <div class="focus">
                        <strong>Fokus Latihan</strong>
                        <div class="chips">
                            <?php foreach (($coach['focus'] ?? []) as $item): ?>
                    ...
                    <div class="cta-row">
                        ...
                        <?php elseif ($isPelanggan): ?>
                            <a class="btn" href="/pages/reservasi.php?coach=<?= urlencode($coach['slug']) ?>">Lanjut ke reservasi</a>
                        <?php else: ?>
                            <a class="btn" href="/pages/dashboard.php">Buka dashboard</a>
                        <?php endif; ?>
                        <a class="btn-outline" href="/index.php#pelatih">Lihat coach lain</a>
                    </div>

                    <section class="section">
                        <h2>Benefit Sesi</h2>
                        <ul>
                            <?php foreach (($coach['benefits'] ?? []) as $benefit): ?>
                    ...
                    <section class="section">
                        <h2>Pengalaman</h2>
                        <ul>
                            <li><?= htmlspecialchars($coach['experience']) ?></li>
                            <li>Sesi cocok untuk pemain dengan target yang ingin naik level lebih terukur.</li>
                            <li>Bisa dipasangkan dengan booking lapangan reguler di <?= htmlspecialchars($company['short_name']) ?>.</li>
                        </ul>
                    </section>
                </article>
                ...
                <aside class="side">
                    <h3>Coach Lainnya</h3>
                    <?php foreach ($relatedCoaches as $related): ?>
                        <a class="side-link" href="/pages/pelatih.php?slug=<?= urlencode($related['slug']) ?>">
                            <strong><?= htmlspecialchars($related['name']) ?></strong>
                            <span><?= htmlspecialchars($related['specialty']) ?> &bull; <?= htmlspecialchars($related['level']) ?></span>
                        </a>
                    <?php endforeach; ?>
                </aside>
            </div>
